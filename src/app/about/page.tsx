...
export default function About() {
  ...
  return (
    <section className="min-h-screen flex flex-col items-center gap-12 px-4 sm:px-6 lg:px-16 py-16 md:py-20 bg-black text-white">
      {/* Top Section */}
      <div className="flex flex-col md:flex-row items-center justify-center gap-10 lg:gap-12 w-full max-w-6xl">
        {/* Profile Image */}
        <motion.div
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
        ...
        >
          <Image
            ...
            width={320}
            height={320}
            className="rounded-2xl border-4 border-green-600 shadow-xl hover:scale-105 transition-transform duration-300 w-64 sm:w-72 md:w-80 lg:w-[350px] h-auto"
          />
        </motion.div>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 1 }}
        className="mt-10 w-full max-w-5xl px-2 sm:px-4"
      >
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-10">
          My <span className="text-green-500">Journey</span>
        </h2>
        <div className="relative border-l-4 border-green-600 pl-4 sm:pl-6 space-y-8 sm:space-y-10">
          {timeline.map((item, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, x: -50 }}
            ...
            >
              <span className="text-green-400 font-semibold text-sm sm:text-base">{item.year}</span>
              <h3 className="text-lg sm:text-xl font-bold mt-2">{item.title}</h3>
              <p className="text-gray-300 mt-2 text-sm sm:text-base">{item.description}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
