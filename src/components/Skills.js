...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-20 px-4 sm:px-6 lg:px-16 bg-black">
      {/* Section Heading */}
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-6 text-white">
        My <span className="text-green-500 underline">Skills</span>
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {categories.map((category, i) => (
          <motion.div
            ...
            transition={{ duration: 0.6, delay: i * 0.2 }}
            viewport={{ once: true }}
            className="bg-gray-900 p-5 sm:p-6 rounded-lg shadow-lg hover:shadow-green-500/30 transform hover:-translate-y-2 transition duration-300"
          >
            {/* Category Title with Icon */}
            <div className="flex items-center gap-2 mb-4">
              {category.icon}
            ...
            {category.skills.map((skill, j) => (
              <div key={j} className="mb-4">
                <div className="flex justify-between text-gray-300 text-xs md:text-sm mb-1">
                  <span>{skill.name}</span>
                  <span>{skill.level}%</span>
                </div>
                <div className="w-full bg-gray-800 rounded-full h-2">
