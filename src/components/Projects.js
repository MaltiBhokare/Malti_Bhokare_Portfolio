...
export default function Projects() {
  const projects = [
    ...
    },
    {
      ...
      tech: ["HTML", "CSS", "JavaScript", "Bootstrap"],
      github:
        "https://github.com/MaltiBhokare/MaltiBhokare-Digitalcraftstudio",
    },
  ];
  ...
  return (
    <section id="projects" className="py-20 bg-black text-white">
      ...
      {/* Projects Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 px-4 sm:px-8 md:px-16">
        {projects.map((p, i) => (
          <motion.div
            key={i}
            initial={{ opacity: 0, y: 40 }}
            ...
            className="bg-gray-900 rounded-xl p-6 shadow-lg border border-gray-700 hover:border-green-500 transition flex flex-col justify-between"
          >
            <div>
              <h3 className="text-xl md:text-2xl font-bold text-green-400 mb-3 text-center sm:text-left">
                {p.title}
              </h3>
              <p className="text-gray-300 text-sm md:text-base mb-4 text-center sm:text-left">
                {p.description}
              </p>
              ...
              <div className="flex flex-wrap justify-center sm:justify-start gap-2 mb-4">
                {p.tech.map((t, index) => (
                  <span
                    key={index}
                    className="px-3 py-1 text-xs md:text-sm bg-green-600/20 text-green-400 rounded-full border border-green-600"
                  >
                    {t}
                  </span>
                ))}
            ...
            <div className="flex justify-center sm:justify-start">
              <a
                ...
                className="inline-block mt-2 px-4 py-2 text-sm md:text-base font-semibold border border-green-500 text-green-500 rounded-lg hover:bg-green-600 hover:text-white transition"
              >
                🔗 View on GitHub
              </a>
            </div>
